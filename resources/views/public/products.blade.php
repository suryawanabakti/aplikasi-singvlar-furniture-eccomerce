...
@section('content')
    <div class="container">
        ...
        <div class="row mt-3 mb-4 button-group filter-button-group">
            <div class="col">
                <a href="/"
                    class="btn {{ Request::is('products') ? 'btn-primary' : 'btn-secondary' }} mx-1 text-white">All
                    Category</a>
                @foreach ($categories as $category)
                    <a href="{{ route('home.category', $category->id) }}"
                        class="btn {{ Request::is('products/home/' . $category->id) ? 'btn-primary' : 'btn-secondary' }} mx-1 text-white text-capitalize">{{ $category->name }}</a>
                @endforeach
            </div>
        </div>
